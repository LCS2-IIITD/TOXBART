...
from transformers import AutoTokenizer, BertTokenizer, BartTokenizer, get_scheduler, BartConfig, Trainer, TrainingArguments, trainer_utils, DataCollatorWithPadding
import pandas as pd
...
```python train.py --dataset_type sbic --threshold 0.5 \\
--output_dir sbic_config5_model --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
--data_file ../../data/SBIC.v2.trn.csv```

For baseline on LatentHatred dataset use this:
```python train.py --dataset_type latent --threshold 0.5 \\
--output_dir latent_config5_model --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
...
def tokenize_data(
    ...
    def tokenize(examples):
        ...
        process_labels(target_tokenized)
        return {**seq2seq_tokenized, **toxic_tokenized, **target_tokenized}

    if 'HITId' in dataset:
        remove_cols = ['HITId', 'post', 'target', 'toxic_labels']
    else:
        remove_cols = ['post', 'target', 'toxic_labels']
    tokenized = dataset.map(
        tokenize, batched=True,
        num_proc=1,
        remove_columns=remove_cols
...
if __name__ == "__main__":
    ...
    model = ToxBARTC4.from_pretrained("facebook/bart-base")

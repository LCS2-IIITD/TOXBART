...
from modeling_toxbart import ToxBARTC4
...
```python test.py --model sbic_config5_model/<checkpoint-directory> --threshold 0.5 \\
--output sbic_config5 --dataset_type sbic --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
--data_file ../../data/SBIC.v2.tst.csv --generate_scores```

For inference on the LatentHatred dataset use this:
```python test.py --model latent_config5_model/<checkpoint-directory> --threshold 0.5 \\
--output latent_config5 --dataset_type latent --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
...
if __name__ == "__main__":
    ...
    model = ToxBARTC4.from_pretrained(args.model)
    ...
    if not os.path.exists(pickle_file):
        ...
        input_signals = get_toxicity_probabilities(df, args.tox_model_dir)

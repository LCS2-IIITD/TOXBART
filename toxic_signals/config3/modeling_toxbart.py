...
class ToxModelC1(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.toxic_gen_space = nn.Linear(6, config.d_model)
    ...
    def forward(
        ...
        projected_toxic_signals = self.toxic_gen_space(toxic_signals)
        projected_toxic_signals = projected_toxic_signals.reshape(-1, projected_toxic_signals.shape[-1])
        concatenated_enc_outputs = torch.cat((encoder_outputs["last_hidden_state"], projected_toxic_signals.unsqueeze(1).repeat(1, encoder_outputs["last_hidden_state"].shape[1], 1)), dim=2)
...
class ToxBARTC1(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = ToxModelC1(config)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        return {
            ...
            "toxic_signals": torch.stack([torch.FloatTensor(each) for each in kwargs["toxic_signals"]]).cuda(),
        }

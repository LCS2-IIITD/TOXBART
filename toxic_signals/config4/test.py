...
from modeling_toxbart import ToxBARTC3
...
```python test.py --model sbic_config4_model/<checkpoint-directory> --threshold 0.5 \\
--output sbic_config4 --dataset_type sbic --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
--data_file ../../data/SBIC.v2.tst.csv --generate_scores```

For inference on the LatentHatred dataset use this:
```python test.py --model latent_config4_model/<checkpoint-directory> --threshold 0.5 \\
--output latent_config4 --dataset_type latent --tox_model_dir ../tox_bert/bert-toxic-signals-probab/<tox-bert-checkpoint-folder> \\
...
def tokenize_data(
    ...
    def tokenize(examples):
        ...
        toxic_tokenized["toxic_input_ids"] = toxic_tokenized.pop("input_ids")
        toxic_tokenized["toxic_attention_mask"] = toxic_tokenized.pop("attention_mask")

        return {**seq2seq_tokenized, **toxic_tokenized}
...
def generate_stereotype(
    ...
    model_kwargs = {'encoder_outputs': encoder_outputs, "toxic_input_ids": batch["toxic_input_ids"], "toxic_attention_mask": batch["toxic_attention_mask"]}
...
if __name__ == "__main__":
    ...
    model = ToxBARTC3.from_pretrained(args.model)
    ...
    toxic_labels = get_toxicity_labels(input_signals, sep_token=tokenizer.sep_token)

    test_ds = Dataset.from_pandas(df)
    signals_ds = Dataset.from_dict({"toxic_labels": toxic_labels})

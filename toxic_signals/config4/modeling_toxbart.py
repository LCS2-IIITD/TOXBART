...
class ToxModelC3(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.concatenated_shared_space = nn.Linear(config.d_model * 2, config.d_model)
    ...
    def forward(
        ...
        concatenated_enc_outputs = torch.cat((encoder_outputs[0], toxic_enc_outputs[0]), dim=2)
        projected_concatenated_enc_outputs = self.concatenated_shared_space(concatenated_enc_outputs)
        encoder_outputs["last_hidden_state"] = projected_concatenated_enc_outputs
...
class ToxBARTC3(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = ToxModelC3(config)
